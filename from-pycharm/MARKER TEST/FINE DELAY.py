...
############################################################################################################################
        ...
        for in_sr in range(len(sampling_rate)):
            ...
            for ii in num_channels:
                ...
                channb_on_scope = 1
                mrkr_on_scope = 2
                horizontal_scale_per_divison = 0.5e-9
                ...
                for marker_number in num_markers:
                    ...
                    inst.send_scpi_cmd(':MARK:SEL {};:MARK:STAT ON'.format(marker_number))
                    inst.send_scpi_cmd(':MARK:VOLT:PTOP 1.0;:MARK:VOLT:OFFS 0')
                    inst.send_scpi_cmd(':MARK:DEL:FINE {}'.format(del_range[in_sr]))
                    time.sleep(1)
                    scope.write(':MEASure:DELTAtime CHANnel{0},CHANnel{1}'.format(channb_on_scope, mrkr_on_scope))
                    time.sleep(5)
                    scope.write(':MEASure:RESults?')
                    ...
                    if diff < l_range[in_sr] or diff > h_range[in_sr]:
                        test_success = False
                        print('Test1 FAIL for fine delay {}ps'.format(float(del_range[in_sr]) / 1e-12))
                    else:
                        print('Test1 pass for fine delay {}ps'.format(float(del_range[in_sr]) / 1e-12))
                    ...
                    if diff < -(h_range[in_sr]) or diff > -(l_range[in_sr]):
                        test_success = False
                        print('Test2 FAIL for fine delay -{}ps'.format(float(del_range[in_sr]) / 1e-12))
                    else:
                        print('Test2 pass for fine delay -{}ps'.format(float(del_range[in_sr]) / 1e-12))
                if (test_success):
                    print('Test pass for marker fine delay at sampling rate {} GSas'.format(
                        int(sampling_rate[in_sr]) / 1e9))
                else:
                    print('Test fail for marker fine delay at sampling rate {} GSAs'.format(
                        int(sampling_rate[in_sr]) / 1e9))
...
test = fine_delay_test('192.90.70.22')
test.execute()