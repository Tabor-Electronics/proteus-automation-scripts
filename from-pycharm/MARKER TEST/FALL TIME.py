...
class fall_time_test:
    ...
    def execute(self):
        ...
        fall_time = 200e-12
        tb_scale = 2.0515e-6 # delaying the signal to get th fall time
        offset_fall = 20e-3 #for accurate measurement
        mrkr_on_scope=2
        for i in range(4):
            channb = i+1
            ...
            for mrker in range(2):
                ...
                scope.write(':TIMebase:POSition {0}'.format(tb_scale))
                time.sleep(1)
                scope.write(':TIMebase:SCALe {0}'.format(horizontal_scale_per_divison))
                time.sleep(1)
                scope.write('CHAN{0}:SCAL {1}'.format(mrkr_on_scope, vertical_scale_per_divison))
                time.sleep(1)
                scope.write(':CHANnel{0}:OFFSet {1}'.format(mrkr_on_scope, offset_fall))
                time.sleep(1)

                scope.write(':MEASure:FALLtime CHANnel{0},80,20'.format(mrkr_on_scope))
                time.sleep(1)
                ...
                if fall_time_temp > fall_time:
                    test_success = False
                    print("TEST FAILED for fall time on channel {} marker {}".format(channb, mrker))
                else:
                    print("TEST PASSED for fall time on channel {} marker {}".format(channb, mrker))
        if (test_success):
            print('Test successed for marker fall time')
        else:
            print('Test failed for marker fall time')
        disconnect()
        scope.close()
test = fall_time_test('192.90.70.22')
test.execute()